{   Program TEST_USB [options]
*
*   Allow testing byte-level I/O to a Embed USB device.
}
program test_sio;
...
var
  usbid: file_usbid_t;                 {VID in high 16 bits, PID in low}
  name:                                {required Embed USB name of device}
...
retry:                                 {back here after reading new chunk into buffer}
  ...
  file_read_embusb (                   {read next chunk of bytes from the device}
    conn,                              {connection to the device}
    ibuf.max,                          {maximum number of bytes to read}
    ibuf.str,                          {buffer of returned bytes}
...
*   Initialize our state before reading the command line options.
  ...
  usbid := lshft(5828, 16) ! 1489;     {init to Embed device 10}
{
...
1: begin
    ...
    if (ii & 16#FFFF) <> ii then goto parm_bad;
    usbid := (usbid & 16#FFFF) ! lshft(ii, 16); {merge in new VID}
    end;
{
*   -PID pid
}
2: begin
    ...
    if sys_error(stat) then goto err_parm;
    if (ii & 16#FFFF) <> ii then goto parm_bad;
    usbid := (usbid & 16#FFFF0000) ! ii; {merge in new PID}
    end;
{
*   -E <embusb number>
}
3: begin
    string_cmline_token_int (ii, stat);
    if (ii < 1) or (ii > 20) then goto parm_bad;
    if ii <= 10
      then begin                       {first block of PIDs}
        ii := (ii - 1) + 1480;
        end
      else begin                       {second block of PIDs}
        ii := (ii - 11) + 2610;
        end
      ;
    usbid := lshft(5824, 16) ! ii;     {set VID and PID}
    end;
{
*   -N name
}
...
*   All done reading the command line.
  ...
  file_open_embusb (                   {open connection to the remote device}
    usbid,                             {VID and PID of the USB device}
...
done_cmd:                              {done processing the current command}
  ...
  if obuf.len > 0 then begin           {one or more bytes to send ?}
    file_write_embusb (obuf.str, conn, obuf.len, stat); {send the data bytes}
    if sys_error(stat) then goto err_cmparm;
    end;
